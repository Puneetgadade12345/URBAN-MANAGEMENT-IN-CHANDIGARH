...
bool authenticateUser(const string& userId, const string& password) {
    // Predefined credentials (In real-world applications, use a secure database)
    const string predefinedUserId = "laxmi";
    const string predefinedPassword = "laxmi@123";

    return userId == predefinedUserId && password == predefinedPassword;
}
...
void addService(vector<string>& services) {
    ...
    if (authenticateUser(userId, password)) {
        ...
        cout << "Authentication failed. You are not authorized to add services.\n";
    }
}
